#!/bin/bash
# Notarizes build/Plain for Mac.app so it opens on any Mac without warnings.
#
...
#        xcrun notarytool store-credentials "plainmac" --apple-id you@example.com --team-id TEAMID
#   3. Build signed:  SIGN_IDENTITY="Developer ID Application: Your Name (TEAMID)" ./build-app.sh
#   4. Run this script.
set -euo pipefail
cd "$(dirname "$0")"
APP="build/Plain for Mac.app"
PROFILE="${NOTARY_PROFILE:-plainmac}"
