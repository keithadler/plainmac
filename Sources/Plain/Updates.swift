...
enum Updates {
    static let repo = "keithadler/plainmac"
    ...
    static func checkAndPresent() {
        Task {
            ...
            case .upToDate:
                ...
                alert.informativeText = String(format: String(localized: "Plain for Mac %@ is the latest version."), CLI.version)
    ...
    private static func present(version v: String, page: URL) {
        let alert = NSAlert()
        alert.messageText = String(format: String(localized: "Plain for Mac %@ is available"), v)
