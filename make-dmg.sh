...
#   ./make-dmg.sh            -> build/Plain-for-Mac-<version>.dmg
...
DMG="build/Plain-for-Mac-$VERSION.dmg"
STAGE=$(mktemp -d)
...
Plain for Mac — how to install

1. Drag the "Plain for Mac" icon onto the "Applications" folder next to it.
...
2. Open the app: open Launchpad (the rocket in the Dock) or your Applications
   folder and click "Plain for Mac".
...
4. Click the Apple menu (top-left) > System Settings > Privacy & Security.
   Scroll all the way down. Next to "Plain for Mac was blocked", click
...
5. Done. It opens like any other app from now on, and macOS never asks again.
   Opens Word, Excel and PowerPoint files, edits the basics, and never damages what it does not understand.
...
https://github.com/keithadler/plainmac
TXT

rm -f "$DMG"
hdiutil create -volname "Plain for Mac" -srcfolder "$STAGE" -ov -format UDZO -quiet "$DMG"
