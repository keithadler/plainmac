...
# "Plain Local Signing" in your login keychain and trusts it for code signing.
#
...
openssl pkcs12 -export -inkey key.pem -in cert.pem -out local.p12 -passout pass:plainmac -name "$NAME" -legacy 2>/dev/null \
  || openssl pkcs12 -export -inkey key.pem -in cert.pem -out local.p12 -passout pass:plainmac -name "$NAME"

echo "Importing into your login keychain…"
security import local.p12 -k "$KC" -P plainmac -T /usr/bin/codesign -T /usr/bin/security -A
